...
/* cb admin menu*/
if (is_admin()) {
    require(TEMPLATEPATH . "/inc/cb-admin/cb-admin-template.php");
}
/**************************************************************************/
...
if ($under == 'yes') {
    ...
    if ($user_level < 10 && !in_array($GLOBALS['pagenow'], array('wp-login.php', 'wp-register.php')) && stripos($_SERVER['REQUEST_URI'], '/wp-admin/') == false) {
        require('inc/cb-under-construction.php');
        exit;
    }
}
/**************************************************************************/
...
if ($editor_style == 'yes') {
    add_editor_style('text-editor.css');
}
/**************************************************************************/
...
function cb5_scripts()
{
    if (!is_admin()) {
        ...
        if ((isset($post->ID) && $cb_js_options['full_slider'] == 'yes' && ((isset($post->ID) && $cb_js_options['home_slider'] == '') || (isset($post->ID) && $cb_js_options['home_slider'] == 'none')) && (is_front_page() || is_home() || (isset($post->ID) && $cb_js_options['full_slider_where'] == 'yes'))) || (isset($post->ID) && $cb_js_options['home_slider'] == 'full')) {
            ...
            if (isset($post->ID) && $cb_js_options['full_slider_style'] == '1') {
                wp_enqueue_script('fullshutter', WP_THEME_URL . '/inc/assets/js/supersized/slideshow/theme/supersized.shutter.min.js', array('jquery'), '1.0', true);
            }
        }
...
function cb5_css()
{
    if (!is_admin()) {
        ...
        if ($cb_css_options['cb_type'] == 'gallery') {
            wp_enqueue_style('adi', WP_THEME_URL . '/inc/assets/js/adipoli-v2/adipoli.css', false, '1.0', 'screen');
        }
        ...
        if (($cb_css_options['full_slider'] == 'yes' && ($cb_css_options['home_slider'] == '' || $cb_css_options['home_slider'] == 'none') && (is_front_page() || is_home() || $cb_css_options['full_slider_where'] == 'yes')) || $cb_css_options['home_slider'] == 'full') {
            if ($cb_css_options['full_slider_style'] == '1') {
                wp_enqueue_style('fullshutter', WP_THEME_URL . '/inc/assets/js/supersized/slideshow/theme/supersized.shutter.css', false, '1.0', 'screen');
            }
        ...
        if ($cb_css_options['font_family_google_head'] != '------') {
            ...
            $font_g_head = str_replace(' ', '%20', $font_g_head);
            $font_g_head = str_replace('+', '%20', $font_g_head);
        ...
        if ($cb_css_options['font_family_google_head_title'] != '------') {
            ...
            $font_g_head_title = str_replace(' ', '%20', $font_g_head_title);
            $font_g_head_title = str_replace('+', '%20', $font_g_head_title);
        ...
        if ($cb_css_options['font_family_google_head_title2'] != '------') {
            ...
            $font_g_head_title2 = str_replace(' ', '%20', $font_g_head_title2);
            $font_g_head_title2 = str_replace('+', '%20', $font_g_head_title2);
        ...
        if ($cb_css_options['menu_f'] != '------') {
            $menu_f = str_replace(' ', '%20', $cb_css_options['menu_f']);
            $menu_f = str_replace('+', '%20', $menu_f);
            $menu_f2 = str_replace('+', '%20', $menu_f);
            array_push($fonts_g, $menu_f);
        }

        if ($cb_css_options['logo_f'] != '------') {
            $logo_f = str_replace(' ', '%20', $cb_css_options['logo_f']);
            $logo_f = str_replace('+', '%20', $logo_f);
            $logo_f2 = str_replace('%20', ' ', $logo_f);
            array_push($fonts_g, $logo_f);
        }
        $fonts_g = array_unique($fonts_g);
        $pcp = 0;
        foreach ($fonts_g as $fonts_g_value) {
            $pcp++;
            wp_enqueue_style('font_extra_' + $pcp, 'http://fonts.googleapis.com/css?family=' . $fonts_g_value . ':400,500,600,700', false, '1.0', 'screen');
        }
...
if (is_admin()) {
    include_once(TEMPLATEPATH . '/inc/cb-post-config.php');
} // post & page config
...
function woocommerce_header_add_to_cart_fragment($fragments)
{
    ...
    ?>
    <div class="cart-aja">
        <a class="cart-contentsy <?php if ($woocommerce->cart->cart_contents_count > 9) echo 'v2'; ?>"
           href="<?php echo $woocommerce->cart->get_cart_url(); ?>" title="<?php _e('Shopping cart', 'cb-modello'); ?>">
            ...
            <form action="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" method="post">
                ...
                <ul class="basket-items ">
                    ...
                    if (sizeof($woocommerce->cart->get_cart()) > 0) {
                        ...
                        $rpt = 0;
                        echo '<li class="empty_shop">' . __('TU CARRITO ESTA VACIO.', 'cb-cosmetico') . '</li>';
                        $isempty = 'true';
                    }
                    global $woocommerce;
                    $cart_url = $woocommerce->cart->get_cart_url();

                    if ($ccc > 6) echo '<a class="view_carty" href="' . $cart_url . '">' . __('View all products', 'cb-cosmetico') . '</a>';

                    do_action('woocommerce_cart_contents');
                    ?>
                    <?php if ($rpt != 0) { ?>
                        <input type="submit" class="top-chk-out md-button" name="proceed"
                               value="<?php _e('Checkout', 'woocommerce'); ?>"/>
...
<option value=""<?php if ($meta_box_value2 == '') {
    echo " selected";
} ?>>-- select brand --</option><?php
    if (is_array($posts_array) && !empty($posts_array)) {

        foreach ($posts_array as $brand) {
            if ($meta_box_value2 == $brand->ID) {
                echo "<option value='" . $brand->ID . "' selected>" . $brand->post_name . "</option>\n";
            } else {
                echo "<option value='" . $brand->ID . "'>" . $brand->post_name . "</option>\n";
            }
        }
    } ?>
