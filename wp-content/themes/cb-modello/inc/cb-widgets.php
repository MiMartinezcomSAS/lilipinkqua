...
class gallwidget extends WP_Widget {
	function gallwidget() {
		...
		$this->WP_Widget('gallwidget', 'cb-gallery', $widget_ops);
	}
...
class testimonialswidget extends WP_Widget {
	function testimonialswidget() {
		...
		$this->WP_Widget('testimonialswidget', 'cb-testimonials', $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
...
<p>
	<label for="<?php echo $this->get_field_id('tit'); ?>"><?php _e('Widget Title','cb-dayandnight'); ?><br />
...
<div class="testimonials_container">
	...
	<?php if($instance['author_text'][0]==''){?>
		...
		<?php _e('Text','cb-dayandnight');?>
			...
			<br />
			<?php _e('Position','cb-dayandnight');?>
			<br /> <input
	...
	</br> <span style="font-size: 10px;color: #777;"><?php _e('Save & Reload before adding fields','cb-dayandnight');?>
...
class clientswidget extends WP_Widget {
	function clientswidget() {
		...
		$this->WP_Widget('clientswidget', 'cb-clients', $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
		$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
function cbrp() {
	...
	$this->WP_Widget('cbrp', 'cb-recent-posts', $widget_ops);
}
function widget($args, $instance) {
extract($args, EXTR_SKIP);
$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
function cbfeat() {
	...
	$this->WP_Widget('cbfeat', 'cb-featured-post', $widget_ops);
}
function widget($args, $instance) {
extract($args, EXTR_SKIP);
$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
function cbmore() {
	...
	$this->WP_Widget('cbmor', 'cb-more-posts', $widget_ops);
}
function widget($args, $instance) {
extract($args, EXTR_SKIP);
$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
function cb5_gmap() {
	...
	$this->WP_Widget('cb5_gmap', 'cb-gmap', $widget_ops);
}
function widget($args, $instance) {
extract($args, EXTR_SKIP);
$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
class cb5_twitter extends WP_Widget {
    function cb5_twitter() {
        ...
        $this->WP_Widget('cb5_twitter', 'cb-twitter', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args, EXTR_SKIP);
        $title = empty($instance['title']) ? '&nbsp;' : apply_filters('widget_title', $instance['title']);
...
class cb5_mailchimp  extends WP_Widget {
    function cb5_mailchimp() {
        ...
        $this->WP_Widget('cb5_mailchimp', 'cb-mailchimp', $widget_ops);
    }
    function widget($args, $instance) {
        extract($args, EXTR_SKIP);
...
class cbwootop extends WP_Widget {
	function cbwootop() {
		...
		$this->WP_Widget('cbwootop', 'cb-wootop', $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
		$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
if($ccc>6)  echo '<a class="view_carty" href="'.$cart_url.'">'.__('View all products','cb-cosmetico').'</a>';
		...
		?>
		<?php if($rpt!=0) {?>
				<input type="submit" class="top-chk-out md-button" name="proceed" value="<?php _e( 'Checkout', 'woocommerce' ); ?>" />
...
class cbwoolog extends WP_Widget {
	function cbwoolog() {
		...
		$this->WP_Widget('cbwoolog', 'cb-woolog', $widget_ops);
	}
	function widget($args, $instance) {
		extract($args, EXTR_SKIP);
		$tit = empty($instance['tit']) ? '&nbsp;' : apply_filters('widget_tit', $instance['tit']);
...
<p>
	<label for="<?php echo $this->get_field_id('welcome'); ?>"><?php _e('Welcome text','cb-cosmetico'); ?><br />
...
<p>
	<label for="<?php echo $this->get_field_id('hotline'); ?>"><?php _e('Hotline Number','cb-cosmetico'); ?><br />
